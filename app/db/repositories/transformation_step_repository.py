...
def get_step_by_id(step_id: UUID) -> TransformationStep:
    ...
    """
    client: Client = get_supabase()
    resp = client.table(TABLE_NAME).select("*").eq("id", str(step_id)).execute()
    data = resp.data
    if not data:
        raise HTTPException(
...
def update_step(
    step_id: UUID, payload: TransformationStepUpdate
    ...
    if "transformation_id" in update_data and update_data["transformation_id"]:
        ...
        get_transformation_by_id(
            update_data["transformation_id"]
        )  # Will raise if not found

    update_data = serialize_for_supabase(update_data)

    resp = client.table(TABLE_NAME).update(update_data).eq("id", str(step_id)).execute()
    data = resp.data
    if not data:
        return None
    return TransformationStep.model_validate(data[0])
